    ...
    <div className="bg-white border border-slate-200 rounded-xl p-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm">Every listing is confirmed as a real, active opening.</p>
        </div>
        <button onClick={() => setOpen(!open)} className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">
          {open ? "Cancel" : "+ New Job"}
        </button>
      </div>
      ...
      {open && (
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Salary (optional)</label>
              <input value={form.salary} onChange={e => setForm({...form, salary: e.target.value})}
                placeholder="e.g. $85,000–$100,000/yr"
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Requirements (optional)</label>
            <textarea rows={3} value={form.requirements} onChange={e => setForm({...form, requirements: e.target.value})}
              placeholder="Licenses, certifications, experience required..."
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div className="flex items-start gap-3 bg-blue-50 rounded-lg p-4">
            <span className="text-blue-600 text-lg">ℹ</span>
            <p className="text-sm text-blue-700">By posting this job, you confirm this is a real, active opening. Fraudulent listings will result in account suspension.</p>
          </div>
